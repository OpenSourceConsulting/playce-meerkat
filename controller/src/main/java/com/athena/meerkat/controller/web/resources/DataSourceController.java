...
public class DataSourceController {
	...
	@ResponseBody
	public SimpleJsonResponse getAvailableDataSource(SimpleJsonResponse json, int tomcatId) {
	...
	@ResponseBody
	public GridJsonResponse getAssociatedTomcatInstances(GridJsonResponse json, @PathVariable Integer dsId) {
		List<TomcatInstance> tomcatInstances = service.getAssocicatedTomcatInstances(dsId);
	...
	@ResponseBody
	public SimpleJsonResponse testConnection(SimpleJsonResponse json, String jdbcUrl, String username, String password, String dbType) {
		// String jdbcUrl = "jdbc://localhost:3306/test";
		// String userName = "root";
		// String password = "admin";
		// String dbType = "MySQL";
		boolean result = service.testConnection(jdbcUrl, username, password, dbType);
	...
	@ResponseBody
	public SimpleJsonResponse saveLinking(SimpleJsonResponse json, String ids, int tomcatId, boolean isRestart) {
		TomcatInstance tomcat = tomcatService.findOne(tomcatId);
		if (tomcat == null) {
			json.setMsg("Tomcat does not exist.");
			json.setSuccess(false);
	...
	@ResponseBody
	public SimpleJsonResponse remove(SimpleJsonResponse json, int tomcatId, int dsId) {
		TomcatInstance tomcat = tomcatService.findOne(tomcatId);
		if (tomcat == null) {
			json.setMsg("Tomcat does not exist.");
			json.setSuccess(false);
