...
public class ClusteringConfigurationController {
	...
	public @ResponseBody SimpleJsonResponse getLatestClusteringVersion(SimpleJsonResponse json, String objectType, Integer objectId) {
		ClusteringConfigurationVersion latestVersion = null;
		Integer groupId = objectId;
		if (objectType.equals(MeerkatConstants.OBJ_TYPE_DOMAIN)) {
			TomcatDomain domain = domainService.getDomain(objectId);
			if (domain != null) {
				groupId = domain.getDataGridServerGroupId();
			}
		}
		latestVersion = service.getLatestClusteringConfVersionByServerGroup(groupId);
		if (latestVersion != null) {
			json.setData(latestVersion.getId());
		}
		return json;
	...
	public @ResponseBody GridJsonResponse getClusteringConf(GridJsonResponse json, String objectType, Integer objectId, @PathVariable Integer version) {
		List<ClusteringConfiguration> confs = new ArrayList<ClusteringConfiguration>();
		DatagridServerGroup group = null;
		if (objectType.equals(MeerkatConstants.OBJ_TYPE_DOMAIN)) {
			TomcatDomain domain = domainService.getDomain(objectId);
			if (domain != null) {
				group = domain.getServerGroup();
			}
		} else {
			group = datagridServerGroupService.getGroup(objectId);
		}
		if (group != null) {
